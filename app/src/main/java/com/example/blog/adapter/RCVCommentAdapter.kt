...
class RCVCommentAdapter(val activity1: Context, val commentList: ArrayList<ReadComment>) :
    RecyclerView.Adapter<RCVCommentAdapter.ViewHolder>() {
    ...
    private fun hideKeyboard(v: View?) {
        val inputMethodManger: InputMethodManager? =
            activity1.getSystemService(Context.INPUT_METHOD_SERVICE) as InputMethodManager?
        inputMethodManger?.hideSoftInputFromWindow(v?.getWindowToken(), 0)
    }

    private fun LayoutDefault(holder: RCVCommentAdapter.ViewHolder) {
